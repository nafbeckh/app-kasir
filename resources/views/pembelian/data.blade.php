...
<script>
    $(document).ready(function() {
        ...
        var table = $('#tableData').DataTable({
            ...
            "buttons": [, {
                ...
                action: function(e, dt, node, config) {
                    window.location.replace("{{ route('pembelian.create') }}")
                }
            }, {
        ...
        $('body').on('click', '#btnDelete', function() {
            id = $(this).data("id");
            let url = "{{ route('pembelian.destroy', ':id') }}";
        ...
        $('body').on('click', '#btnDetail', function() {
            ...
            id = $(this).data('id');
            let url = "{{ route('pembelian.edit', ':id') }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                method: 'GET',
                success: function(res) {
                    // console.log(res)
                    for (i = 0; i < res.data.pembelian_detail.length; i++) {
                        tbdetail.row.add([
                            res.data.pembelian_detail[i].produk.kode_prod,
                            res.data.pembelian_detail[i].produk.nama_prod,
                            res.data.pembelian_detail[i].harga_beli,
                            res.data.pembelian_detail[i].jumlah,
                            res.data.pembelian_detail[i].subtotal,
                        ]).draw()
                    }
        ...
        $('#formDeleteBatch').submit(function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('pembelian.destroy.batch') }}",
                data: $(form).serialize(),
                beforeSend: function() {
                    console.log('otw')
                },
